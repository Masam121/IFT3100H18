...
\state
L'application utilise un algorithme d'occlusion de pour tenter de minimiser le nombre d'éléments du graphe de scène à rendre du point de vus d'une caméra (ex: algorithmes de type BVH, BSP, quadtree, octree, kd-tree, etc). Les techniques d'occlusion de bases sont exclues pour ce critère (ex: faces inversées, occlusion du frustum de vision, algorithme du peintre, z-buffer).
...
\state
Un moins un effet visuel présent dans une scène est réalisé à partir d'une caméra qui rend dans une texture du contenu qui est ensuite affiché dans une scène rendue par une autre caméra (ex: portail, mirroir, effet de réflexion, écran présent dans une scène où est affichée le contenu d'un autre point de vue de la scène, etc). 
