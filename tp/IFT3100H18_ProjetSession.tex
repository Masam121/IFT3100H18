...
\state
Il est possible d'appliquer au moins 3 types d'effet visuel sur toute la surface de la fenêtre d'affichage (ex: anti-crénelage, occlusion ambiante, filtres de convolution, post-process, etc).
...
\state
Il est possible de rendre au moins 1 modèle où une texture est utilisée pour moduler l'orientation de la normale afin de simuler un effet de relief sur la surface, comme par exemple les techniques du \textit{normal mapping} ou du \textit{parallax mapping}.
...
\state
Il existe au moins un modèle d'illumination basé sur une fonction BRDF et qui
n'est pas simplement un des modèles d'illumination classique (ex: phong) exprimés
sous forme d'une fonction BRDF.
...
\state
Il est possible de rendu au moins 1 scène avec une architecture de rendu en différé.
